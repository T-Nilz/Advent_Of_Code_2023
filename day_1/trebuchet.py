...
####### QUESTION 1 #######
# Split the string by newline characters to get individual strings
input_list = input_strings.strip().split("\n")


def find_first_and_last_int(input_list: list) -> list:
    """
    Search first and last digit in list of string, then add them.
    If there is only one digit, it is considered the first and the last.
    They are then combined as a string then summed.

    Ex : 2htzsvdhvqvdjv -> 22
         3slmzvnine5fm  -> 35  --> summed -> 73
         eightone16nine -> 16

    Args:
        input_list (List): List of strings to parse
    Returns:
        int: Sum according to whats explained in the dosctring
    """
    output_list = []
    for statement in input_list:
        ...
        for char in statement:
            try:
                int(char)
                stmt_list.append(char)
            except ValueError:
                continue
        output_list.append(int(stmt_list[0] + stmt_list[-1]))
    return sum(output_list)


print(find_first_and_last_int(input_list))
####### QUESTION 2 #######
...
def sum_first_and_last_digit_enhanced(input_list: list) -> int:
    """
    Concatenate the first and last digit of a string, taking into
    account if an intenger if written in letters.
    Then sum all the int found in the list.

    Ex : 
        2htzsvdhvqvdjv -> 2htzsvdhvqvdjv -> 22
        3slmzvnine5fm  -> 3slmzv95fm     -> 35    --> summed -> 146
        eightone16nine -> 81169          -> 89 
    ...
    for statement in input_list:
        ...
        for idx, char in enumerate(statement):
            ...
            for num in number_dict.keys():
                if statement[idx:].startswith(num):
                    stmt_list.append(number_dict[num])
...
print(sum_first_and_last_digit_enhanced(input_list))T
