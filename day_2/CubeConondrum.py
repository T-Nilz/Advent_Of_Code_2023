...
game_input = game_input.strip().split("\n")


def db_games_result(game_input: list) -> dict:
    """
    Rewrite the input of the game as a dict.

    {"1":
        {
            "subset_0": {"blue": 4},
            "subset_1": {"green": 1, "red": 2},
            "subset_2": {"blue": 4, "green": 1, "red": 6},
        }
    ...
    }

    Args:
        game_input (list): Input for the game as a list of strings.

    Returns:
        dict: dict described in the docstring above.
    """
    game_dict = {}
    for game in game_input:
        game_id = game.split(":")[0].split()[-1]
        game_dict[game_id] = {}
        for idx, subset in enumerate(game.split(":")[-1].split(";")):
            game_dict[game_id][f"subset_{idx}"] = {}
            for cubes in subset.split(","):
                game_dict[game_id][f"subset_{idx}"][cubes.split()[-1]] = int(
                    cubes.split()[0].strip()
                )
...
game_dict = db_games_result(game_input)

cubes_thresholds = {"red": 12, "green": 13, "blue": 14}


####### QUESTION 1 #######
def check_games(game_dict: dict, cubes_thresholds: dict) -> int:
    """
    Check if the games played are compatible by the defined thresholds.
    ...
    total_numbers = [4*6*1, ...]
    output = sum(total_numbers)

    Args:
        game_dict (dict): Games database as dict
        cubes_thresholds (dict): Dict of the cube colors thresholds

    Returns:
        int: Sum of all the possible games ID
    """
    possible = True
    possible_games = []
    for game_id, subsets in game_dict.items():
        for subset in subsets.keys():
            for cube_color, cube_nbr in cubes_thresholds.items():
                if cube_color in subsets[subset].keys():
                    if subsets[subset][cube_color] > cube_nbr:
                        possible = False
                        break
    ...
    possible_games_as_int = list(map(int, possible_games))

    return sum(possible_games_as_int)


result = check_games(game_dict, cubes_thresholds)
print(f"Total number is {result}")


####### QUESTION 2 #######
def minimum_cubes_per_game(game_dict: dict) -> int:
    """
    Find the minimum number of cubes necessary to play a game.
    Once those number are found for each color, they are then multiplied.
    ...
    Returns:
        int: Sum all the multiplied numbers
    """
    total_number = []
    for game_id, subsets in game_dict.items():
        min_cubes = {"red": 0, "green": 0, "blue": 0}
        for subset in subsets.keys():
            for color, cube_nbr in subsets[subset].items():
                min_cubes[color] = max(min_cubes[color], cube_nbr)

        total_number.append(min_cubes["red"] * min_cubes["green"] * min_cubes["blue"])
        min_cubes = {"red": 0, "green": 0, "blue": 0}

    return sum(total_number)


result1 = minimum_cubes_per_game(game_dict)
print(f"Total number is {result1}")
